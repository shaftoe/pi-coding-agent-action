...
function main() {
  ...
  if (!version) {
    console.error(
      'No version provided. Pass it as an argument or set npm_package_version env var.'
    );
    process.exit(1);
  }
  ...
  if (updated === readme) {
    console.info('No pinned version reference found to update in README.md');
    return;
  }

  writeFileSync(README_FILE, updated, 'utf-8');
  console.info(`Updated README.md pinned version reference to v${version}`);
}
