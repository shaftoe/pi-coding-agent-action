...
export async function createPullRequest(
  ...
  if (base) {
    ...
    baseBranch = base;
    console.info(`[create_pull_request] Using provided base branch: ${baseBranch}`);
  } else if (github.context.payload.repository?.default_branch) {
    // Available in context
    baseBranch = github.context.payload.repository.default_branch;
    console.info(`[create_pull_request] Using default branch from context: ${baseBranch}`);
  } else {
    // Fetch from GitHub API
    console.info(`[create_pull_request] Fetching repository default branch from GitHub API...`);
    ...
    console.info(`[create_pull_request] Fetched default branch: ${baseBranch}`);
  }
